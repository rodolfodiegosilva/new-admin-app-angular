...
declare let $:any;
@Component({
  selector: 'app-searchand-list-meet',
  templateUrl: './searchand-list-meet.component.html',
  styleUrls: ['./searchand-list-meet.component.css'],
  providers: [MessageService,DefaultDataService,ManageuserService,ClassificationsService,Ng4LoadingSpinnerService,TestcreateService,MeetcreateService,QuestiondetailedviewService],
})
export class SearchandListMeetComponent implements OnInit {
 /********for producation *********** */
...
rangeClicked(range) {
 ...
 ngOnInit() {
  ...
  //  this.getLessons();
   ...
   this.getMeetSearchList();
   this.getMeetSearchListCount();
   this.testIdsList      = [];
   this.countryList      = [];
   this.courseList       = [];
   this.classList        = [];
 ...
 getMeetids(){
   ...
   this._meetcreateService.getMeetids(this.data).subscribe(
     data => {
       this.response = data.response;
       for (let tid of this.response) {
         this.testIdsList.push({ "label": tid.meeting_id, "value": tid.meeting_id });
         } 
     },
     error => {}
   );
 ...
 getDate(event:any,column:any){  
   ...
   if(column=='verified_date'){
     this.vestartDate = this.syear+'-'+this.smonth+'-'+this.sdate;
     this.veendDate   = this.eyear+'-'+this.emonth+'-'+this.edate;
   } 
   if(column=='cancelled_date'){
     this.cestartDate = this.syear+'-'+this.smonth+'-'+this.sdate;
     this.ceendDate   = this.eyear+'-'+this.emonth+'-'+this.edate;
   } 
   if(column=='deleted_date'){
     this.destartDate = this.syear+'-'+this.smonth+'-'+this.sdate;
     this.deendDate   = this.eyear+'-'+this.emonth+'-'+this.edate;
   } 
...
setFilter(){
 ...
 getMeetSearchList(){
   this.data = {
     ...
     "meeting_id"      : this.seltdtestIdsList,
     ...
     "vestartDate"     : this.vestartDate,
     "veendDate"       : this.veendDate,
     "cestartDate"     : this.cestartDate,
     "ceendDate"       : this.ceendDate,
     "destartDate"     : this.destartDate,
     "deendDate"       : this.deendDate,
     "limit"           : this.limit,
     "offset"          : this.offset
   }
   this.searchLoader = true;
   this._meetcreateService.getMeetSearchList(this.data).subscribe(
     data  => {
 ...
 getMeetSearchListCount(){
  this._meetcreateService.getMeetSearchListCount(this.data).subscribe(
    data  => {
 ...
 VerifyTestConfirm(){
   if(this.selectedTestid && this.selectedTestid.length>0){
     ...
     this.textonModel = 'Do you really want to verify these records? This process cannot be undone!';
      ...
      this.textonModel = 'Please select meeting to verify!';
    }
 }
 deleteTestConfirm(){
  if(this.selectedTestid && this.selectedTestid.length>0){
    this.showSubBtn = true;
    this.textonModel = 'Do you really want to delete these records? This process cannot be undone!';
   }else{
     this.showSubBtn = false;
     this.textonModel = 'Please select meeting to delete!';
   }
}
cancelTestConfirm(){
  if(this.selectedTestid && this.selectedTestid.length>0){
    this.showSubBtn = true;
    this.textonModel = 'Do you really want to block these records? This process cannot be undone!';
   }else{
     this.showSubBtn = false;
     this.textonModel = 'Please select meeting to block!';
   }
}
CancelMeet(){
  this.data = {
    "inst_id" : this.inst_id,
    "meeting_id" : this.selectedTestid,
...
DeleteMeet(){
 ...
 VerifyMeet(){
   this.data = {
     "inst_id" : this.inst_id,
     "meeting_id" : this.selectedTestid,
     "verified_by": this.Check_admin_id
   }    
   //return false;
